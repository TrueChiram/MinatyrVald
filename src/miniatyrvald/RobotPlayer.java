	...
	static void runArchon() throws GameActionException {
		...
		while (true) {
			...
			try {
				...
				// Always build to the right, if not build to the left

				Direction right = new Direction(0);
				Direction left = new Direction(1);
				// Randomly attempt to build a gardener in this direction
				if (rc.canHireGardener(right) && rc.isBuildReady()) {
					rc.hireGardener(right);
				} else if (rc.canHireGardener(left) && rc.isBuildReady()) {
	...
	static void runGardener() throws GameActionException {
		...
		while (true) {
			...
			try {
				...
				TreeInfo[] senseNearbyTrees = rc.senseNearbyTrees();
				for (TreeInfo t : senseNearbyTrees) {
					if (rc.canWater()) {
						if (t.team == rc.getTeam()) {
							if (rc.canWater(t.ID)) {
								rc.water(t.ID);
							}
						}
				...
				// direction
				if (rc.canBuildRobot(RobotType.SOLDIER, dir) && Math.random() < .3 && rc.isBuildReady()) {
					rc.buildRobot(RobotType.SOLDIER, dir);
				} else if (rc.canBuildRobot(RobotType.LUMBERJACK, dir) && Math.random() < .6 && rc.isBuildReady()) {
